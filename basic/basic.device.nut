t RUNLOOP_INTERVAL = 3;     // check for tag every 3 seconds
const SPICLK = 15000;           // SPI Hz
const DEBUG = 1;                // log
...
class PN532 {
    ...
    function nfc_init() {
        ...
        if (!nfc_get_firmware_version()) {
            log("Didn't find PN53x board");
            booted = false;
        }
    
        if (!nfc_SAM_config()) {
            log("SAM config error");
            booted = false;
        }
    ...
    function nfc_get_firmware_version() {
        ...
        log(format("NFC chip: PN5%02x", response_buffer[6]));
    ...
    function nfc_power_down() {
        ...
        if (!send_command_check_ready([
            ...
            log("Unknown error detected during nfc_power_down");
            return false;
        }
    ...
    function nfc_p2p_target() {
        ...
        if (!send_command([
            ...
            log("Unknown error detected during nfc_p2p_target");
            return false;
        }
    }
...
class SpiFlash {
    ...
    function chipErase() {
        ...
        log("Device: Done with chip erase");
        return 0;
    }
    ...
    function erasePlayBlocks() {
        log("Device: clearing playback flash sectors");
    ...
    function eraseRecBlocks() {
        log("Device: clearing recording flash sectors");
...
log(format("Flash Ready, mfg ID: 0x%02x, dev ID: 0x%02x", flash.mfgID, flash.devID));
flash.sleep();
...
/***************************** Application Code ****************************/
// Configure buttons + assign callbacks
btn1.configure(DIGITAL_IN, function(){ 
    //btn1 controls the LED
    local state = btn1.read();
    led.write(state);
});

btn2.configure(DIGITAL_IN, function(){ 
    // btn2 logs the state
    local state = btn2.read();
    server.log(state);
});

// Useful calls:
therm.read();               // returns temp in Kelvin
therm.read_c();             // returns temp in Celcius
therm.read_f();             // returns temp in American

battery.read_voltage();     // returns battery's voltage
battery.charge_status();    // returns whether the battery is being charged

led.write(LED_ON);          // turns the LED on
led.write(LED_OFF);         // turns the LED off
...
function run_loop() {
    ...
    // if there is
    if (tag_id != null) {
        // send the tag_id to the agent
        agent.send("nfc", tag_id)
        flash_led_for_tag();
    }
